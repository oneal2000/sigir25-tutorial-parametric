...
def main(args):
    ...
    for filename, fulldata in data_list:
        ...
        ret, start_with = read_complete(predict_file) # check completed predictions
        ...
        for test_id, data in tqdm(enumerate(fulldata), total=len(fulldata)):
            ...
            assert test_id == len(ret), f"test_id {test_id} != len(ret) {len(ret)}" # make sure test_id equals to the length of completed predictions
            ...
            if args.inference_method == "icl":
                ...
                ret.append(get_pred(model, psgs=None if args.inference_method == "prag" else passages)) # PRAG without passages
