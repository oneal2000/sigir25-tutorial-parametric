# Adopted from DyPRAG:https://github.com/Trae1ounG/DyPRAG
import os
import gc
import json
import argparse
...
def prepare_training_data_multi_datasets(args, tokenizer, datasets):
    ...
    for dataset_name in datasets:
        ...
        if dataset_name == "popqa":
            ...
            num_train_epochs = 1
        projector = True # all projector dataset

        data_list = load_data(dataset_name, None, args.augment_model, data_dir="./data_aug_projector")
            ...
            for test_id in tqdm(sample_data):
                ...
                for pid, aug in enumerate(augment):
                    adapter_path = os.path.join(
                        ...
                        f"aug_model={args.augment_model}_projector", # projector
                        filename,
                        f"data_{test_id}",
                        f"passage_{pid}"
                    )
    ...
    def __call__(self, examples: List[Dict[str, dict]]) -> Dict[str, torch.Tensor]:
        ...
        for example in examples:
            ...
            with torch.no_grad():
                ...
                input_embeds.append(output.hidden_states[-1][:,-1,:]) # si
...
def main(args):
    ...
    print(f"initialize projector with {args.projector_p} hidden layers") # 32 hidden layers
    ...
    for epoch in range(DYPRAG_TRAIN_EPOCH):
        for step, batch in enumerate(train_dataloader):
            ...
            print(f"Epoch {epoch}, Step {step}, MSE Loss: {mse_loss.item():.4f}, LM Loss: {lm_loss.item():.4f}, KL Loss: {kl_loss.item():.4f}, Target LM Loss: {lm_origin_loss.item():.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets", type=lambda s: s.split(","), default=["2wikimultihopqa", "hotpotqa", "popqa", "complexwebquestions"])
